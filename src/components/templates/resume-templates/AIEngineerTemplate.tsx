...
export default function AIEngineerTemplate({ editorState }: TemplateProps) {
  const { content, visibleSections, sectionOrder = ['summary', 'projects', 'experience', 'education', 'skills', 'certifications', 'achievements', 'awards', 'publications', 'leadership'], theme } = editorState;
  ...
  // Modern cyber/tech-focused styling
  const wrapperStyle = {
    display: 'flex',
    fontFamily: theme.fontFamily || 'Inter, monospace, sans-serif',
    ...
    switch (sectionId) {
      case 'skills':
        return visibleSections.skills !== false && skills.length > 0 ? (
          <div key="skills">
            <h3 style={sidebarSectionTitleStyle}>[0x03] Skills</h3>
                  ...
                  <div key={idx}>
                    ...
                    <div>
                      {cat.skills.map((s: string, sIdx: number) => (
                        <span key={sIdx} style={skillBadgeStyle}>{s}</span>
                      ))}
                    </div>
                  </div>
                ))}
  ...
  return (
    <div style={wrapperStyle} className="w-full">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '8pt', fontFamily: 'monospace', color: '#38bdf8' }}>
            ...
            {personalInfo.contact.github && (
              <a href={personalInfo.contact.github} target="_blank" rel="noreferrer" style={{ color: '#38bdf8', textDecoration: 'none' }}>→ github.com</a>
            )}
            {personalInfo.contact.website && (
              <a href={personalInfo.contact.website} target="_blank" rel="noreferrer" style={{ color: '#38bdf8', textDecoration: 'none' }}>→ portfolio.web</a>
            )}
          </div>
        </div>
      ...
      {/* RIGHT MAIN PANEL (summary, projects (FIRST!), experience, education) */}
