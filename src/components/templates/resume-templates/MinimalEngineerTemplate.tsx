...
export default function MinimalEngineerTemplate({ editorState }: TemplateProps) {
  ...
  // Extremely dense, compact styling for engineering resumes
  const wrapperStyle = {
    fontFamily: theme.fontFamily || 'SFMono-Regular, Consolas, "Liberation Mono", Menlo, monospace, sans-serif',
    fontSize: theme.fontSize || '8.5pt',
    color: '#1e293b',
    ...
    <div style={wrapperStyle} className="w-full">
      ...
      <header style={headerStyle}>
        <div>
          ...
          {personalInfo.title && (
            <div style={{ fontSize: '8.5pt', fontWeight: 600, color: theme.accentColor || '#0284c7', marginTop: '1px' }}>
              {personalInfo.title}
            </div>
          )}
        </div>
              ...
              return skills.length > 0 ? (
                <section key="skills">
                  ...
                  {Array.isArray(skills) && typeof skills[0] === 'string' ? (
                    <p style={{ margin: '0' }}>{(skills as string[]).join(' | ')}</p>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                      {(skills as any[]).map((cat, idx) => (
                        <div key={idx} style={{ display: 'flex', alignItems: 'start' }}>
                          <span style={{ fontWeight: 'bold', width: '120px', flexShrink: 0 }}>{cat.category}:</span>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }} data-section-list="true">
                    {experience.map((exp) => (
                      <div key={exp.id} data-section-item="true">
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', fontSize: '8pt', color: theme.accentColor || '#0284c7', fontStyle: 'italic', marginBottom: '2px' }}>
                          <span>{exp.company}</span>
                          {exp.location && <span>{exp.location}</span>}
                        </div>
                        <ul style={{ listStyleType: 'disc', paddingLeft: '14px', margin: '0', display: 'flex', flexDirection: 'column', gap: '1.5px' }}>
                          {exp.bullets.map((bullet, idx) => (
                            <li key={idx} style={{ fontSize: '8.5pt' }}>{bullet}</li>
                          ))}
                        </ul>
                      </div>
                    ))}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }} data-section-list="true">
                    {projects.map((proj) => (
                      <div key={proj.id} data-section-item="true">
                        ...
                        {proj.description && <div style={{ fontSize: '8pt', fontStyle: 'italic', margin: '1px 0' }}>{proj.description}</div>}
                        <ul style={{ listStyleType: 'circle', paddingLeft: '14px', margin: '0', display: 'flex', flexDirection: 'column', gap: '1.5px' }}>
                    ...
                    {certifications.map((cert) => (
                      <div key={cert.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                        <div>
                          ...
                          <span style={{ fontSize: '8pt', color: '#64748b', marginLeft: '4px' }}>— {cert.issuer}</span>
                        </div>
                        <span style={{ fontSize: '8pt', color: '#64748b' }}>{cert.date}</span>
                      </div>
                    ))}
                  </div>
                </section>
